...
example : AntiSymmetric (· ∼ ·) := by
  ...
  have temp := calc
    y ≡ x + 1 [ZMOD 5] := hx
    _ ≡ (y + 1) + 1 [ZMOD 5] := by rel [hy]
    _ = y + 2 := by ring
  exfalso
  clear hy hx x
  cases' temp with w hw
  have temp : y - (y + 2) = -2 := by ring
  rw [temp] at hw
  clear temp
  have H : ¬5 ∣ -2
  . clear hw y w
...
example : AntiSymmetric (· ≺ ·) := by
  dsimp [AntiSymmetric]
  intro x y hx hy
  constructor <;> apply le_antisymm <;> exhaust
